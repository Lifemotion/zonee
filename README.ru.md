...
API Zone.eu допускает **60 запросов в минуту** на IP-адрес. Утилита автоматически повторяет запросы при HTTP 429, но пользуйтесь ответственно:

- Не опрашивайте API в цикле -- распространение DNS занимает время и не отслеживается через этот API
- `zonee dns list <domain>` (все типы) делает ~11 запросов за раз -- указывайте конкретный тип, когда возможно
- Используйте `--dry-run` для проверки команд перед выполнением
- Не перезапрашивайте данные, которые уже есть
